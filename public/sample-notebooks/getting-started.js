...
// ---
// Adapted from tangent-to/ds examples/getting-started.js. Every number in
// the prose was computed by running these cells on the vega-datasets copy
// of the Palmer penguins (333 complete cases).
...
// Along the way we use [Arquero](https://idl.uw.edu/arquero/) for the data
// handling and Observable Plot (preloaded as `Plot`) for the charts.
...
// three species measured at Palmer Station, Antarctica, here from the
// vega-datasets copy on jsDelivr. One bird has `"."` recorded as its sex and
// ten are unknown; after dropping those we keep 333 complete cases:
// 146 Adelie, 68 Chinstrap, and 119 Gentoo.
...
// variance by default), `stddev`, quantiles, and more, all taking plain
// arrays — `penguins.array(column)` hands them straight out of the Arquero
// table. `ds.stats` adds correlation with an inference layer attached. Body
// mass averages 4207 g with a standard deviation of 805 g, and flipper
// length and body mass move together strongly (r = 0.873, with a p-value so
// small it underflows to 0): bigger flippers, heavier bird.
...
// the strong flipper-mass relationship visible at a glance. Plot reads the
// Arquero table directly. The three species occupy different regions:
// Gentoo (large) sit in the upper right.
...
// ## A two-sample t-test
//
// Do Adelie and Gentoo differ in body mass? We filter the table by species,
// take the mass column from each, and run an independent two-sample t-test
// (matching scipy's `ttest_ind`). The difference is enormous: Adelie average
// 3706 g against Gentoo's 5092 g, t = -23.5 on 263 degrees of freedom, with
// a p-value far below any conventional threshold (it too underflows to 0).
...
// R's `prcomp(x, scale. = TRUE)`. The first component captures 68.6% of the
// variance and the first two together 88.1%. `transform` projects the rows
// onto the components, giving low-dimensional scores to plot.
...
// %% [markdown]
// `ds.plot.ordiplot` draws the scores on the first two components. Coloured
// by species, Gentoo separate cleanly along PC1 — overall size, with flipper
// length (loading 0.96) and body mass (0.91) heaviest on it — while Adelie
// and Chinstrap split along PC2, which the loading arrows tie to the beak:
// depth (0.70) and length (0.53).
...
// ## K-means clustering
//
// `ds.ml.KMeans` follows the same object pattern. We cluster the
// standardized measurements with `k: 3`, never showing the model the species
// labels. K-means only finds a local optimum, and which one depends on the
// random start, so we run ten seeded restarts and keep the one with the
// lowest inertia (the within-cluster sum of squares it minimises). We then
// ask how well the found clusters agree with the true species using the
// adjusted Rand index (1 = perfect agreement, 0 = chance). One cluster is
// exactly the 119 Gentoo; the other two split the Adelie-Chinstrap mass, so
// the agreement is strong but not perfect (ARI = 0.80).
...
// species) shows the same three clouds as the PCA plot. `assign` bolts the
// cluster labels onto the table as a new column.
...
// millimetre of flipper predicts about 50 g more body mass (intercept
// -5872.1, slope 50.15 — the values `lm` reports in R on the same 333
// birds). The `coefficients` array is ordered intercept first, then one
// slope per feature.
...
// %% [markdown]
// Overlaying the fitted line closes the loop: the OLS fit tracks the cloud,
// so body mass rises steadily with flipper length. A straight line only
...
// fits 100 trees on the remaining 266, and `ds.ml.metrics` scores the 67
// held-out predictions. The measurements are highly informative: accuracy on
// this test set is 0.985 — a single mislabelled bird — and Cohen's kappa
// (chance-corrected agreement, which accounts for the unequal class sizes)
// is 0.976.
...
// %% [markdown]
// The confusion matrix shows where the mistakes live: the diagonal counts
// correct predictions per species, and off-diagonal cells are confusions.
...
// decisions. Flipper length (0.41) and beak length (0.39) carry almost all
// of it, roughly tied at the top; beak length is what separates Adelie from
// Chinstrap, the pair the size-driven features cannot tell apart. Body mass
// adds almost nothing (0.06) once the others are in — the same story the
// PCA loadings told above.
...
// clustering, imputers, and cross-validation; `ds.stats` has ANOVA, Tukey
// HSD, nonparametric tests, and model comparison; `ds.mva` adds LDA, CCA,
// and RDA. Each has a matching notebook in the ds repository's examples
// folder.
//
